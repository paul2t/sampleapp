...
set CommonCompilerFlags=-MD -nologo -fp:fast -Gm- -GR- -EHa- -Oi -WX -W4 -DAPP_WIN32=1 -DAPP_NAME=\"%program%\" -FC -Z7 %GraphicsBuildFlags% %IgnoredWarnings% /I..\code /I..\code\glfw /I..\code\imgui
...
if %builderror% == 0 if %RELEASE% == 1 (
	echo Setting icon
	ResourceHacker.exe -open %program%.exe -save %program%.exe -action addskip -res ..\code\resources\icon.ico -mask ICONGROUP,MAINICON,
)
